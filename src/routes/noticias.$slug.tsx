...
import { ArrowLeft, CalendarDays, Clock, User, Link2, Check, Facebook, Linkedin, Instagram, MessageCircle } from "lucide-react";
...
function NewsDetail() {
  ...
  return (
    <>
      {/* HERO — fotografia principal em fundo, conteúdo à esquerda */}
      <header className="relative min-h-[420px] md:min-h-[520px]">
        ...
        <div aria-hidden className="absolute inset-0 bg-gradient-to-r from-[color:var(--navy)]/90 via-[color:var(--navy)]/70 to-[color:var(--navy)]/30" />
        <CornerOrnament aria-hidden className="pointer-events-none absolute right-6 top-6 h-20 w-20 text-[color:var(--gold)]/70" />
        <div className="container-page relative flex min-h-[420px] flex-col justify-end pb-10 pt-28 md:min-h-[520px] md:pb-16 md:pt-32">
          <Link
            to="/noticias"
            className="inline-flex w-fit items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-[color:var(--cream)]/85 hover:text-[color:var(--gold)]"
          >
            <ArrowLeft className="h-4 w-4" aria-hidden /> Voltar para Notícias
          </Link>
          <div className="mt-5 flex flex-wrap items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-white/10 px-3 py-1 text-[11px] font-medium text-[color:var(--cream)]/90">
              <CalendarDays className="h-3 w-3" aria-hidden /> {dateFmt}
            </span>
          </div>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-xs text-[color:var(--cream)]/75">

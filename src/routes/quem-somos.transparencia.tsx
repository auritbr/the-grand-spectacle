...
function Transparencia() {
  ...
  return (
    <main className="min-h-screen bg-[#F6F8FB]">
      ...
      <header className="container-page pt-10 pb-8 text-center md:pt-[52px] md:pb-[42px]">
        <div className="inline-flex items-center gap-1.5 text-[color:var(--navy)]">
          <Archive className="h-4 w-4" aria-hidden />
          <span className="text-[12px] font-semibold uppercase tracking-[0.24em]">Acervo</span>
        </div>
        <h2 className="mt-[10px] font-display text-[27px] font-bold leading-tight text-[color:var(--navy)] md:text-[36px]">
          Acervo institucional
        </h2>
        <RopeCurve aria-hidden className="mx-auto mt-[10px] h-2.5 w-[72px] text-[color:var(--gold)]" />
        <p className="mx-auto mt-[16px] max-w-[720px] text-[15px] leading-[1.5] text-[color:var(--muted-foreground)] md:text-[16px]">
      ...
      <section className="container-page pb-16 md:pb-20">
        <div className="mx-auto w-full max-w-[1200px] space-y-3">
          {GROUP_ORDER.map((g) => {
            ...
            return (
              <div
                key={g}
                className="overflow-hidden rounded-[16px] border border-slate-200 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]"
              >
                <button
                  type="button"
                  onClick={() => toggleGroup(g)}
                  aria-expanded={open}
                  className="flex min-h-[72px] w-full items-center gap-3 px-5 py-[14px] text-left md:px-6"
                >
                  <span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-slate-100 text-[color:var(--navy)]">
                    <Archive className="h-4 w-4" aria-hidden />
                  </span>
                  <span className="min-w-0 flex-1 md:flex md:items-baseline md:gap-2">
                    <span className="block truncate text-[16px] font-semibold text-[color:var(--navy)] md:text-[17px]">
                      {g}
                    </span>
                    <span className="mt-0.5 block text-[13px] text-slate-500 md:mt-0 md:before:mr-2 md:before:content-['·']">
                      {docs.length} {docs.length === 1 ? "documento" : "documentos"}
                    </span>
                  </span>
                  <ChevronDown
                    className={`h-[18px] w-[18px] shrink-0 text-slate-500 transition-transform ${open ? "rotate-180" : ""}`}
                    aria-hidden
                  />
                </button>
                {open && (
                  <div className="border-t border-slate-100">
                    {docs.length === 0 ? (
                      <p className="px-5 py-5 text-sm text-slate-500 md:px-6">
                        ...
                        {docs.map((d) => (
                          ...
                          >
                            <div className="flex min-w-0 items-start gap-3">
                              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-slate-50 text-[color:var(--navy)]">
                                <FileText className="h-4 w-4" aria-hidden />
                              </span>
                              <div className="min-w-0">
                                <p className="truncate text-[15px] font-semibold text-[color:var(--navy)] md:text-[16px]">{d.title}</p>
                            ...
                            <div className="flex shrink-0 gap-2 md:justify-end">
                              <a
                                ...
                                className="inline-flex h-9 items-center gap-1.5 rounded-full border border-slate-200 px-4 text-[13px] font-semibold text-[color:var(--navy)] hover:bg-slate-50"
                              >
                              ...
                              <a
                                href="#"
                                onClick={(e) => e.preventDefault()}
                                className="inline-flex h-9 items-center gap-1.5 rounded-full bg-[color:var(--navy)] px-4 text-[13px] font-semibold text-white hover:brightness-110"
                              >
        ...
        {/* CTA FINAL */}
        <div className="mx-auto mt-14 w-full max-w-[1200px] md:mt-16">
